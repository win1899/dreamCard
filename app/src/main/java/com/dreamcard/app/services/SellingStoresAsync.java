...
/**
 * Created by WIN on 2/26/2016.
 */
public class SellingStoresAsync extends AbstractAsyncTask<Object, Void, Object> {
    ...
    public SellingStoresAsync(IServiceListener listener, ArrayList<ServiceRequest> list, int processType){
    ...
    private Object callService(){
        ...
        SoapObject request = new SoapObject(ServicesConstants.WS_NAME_SPACE, ServicesConstants.WS_METHOD_ALL_SELLING_STORES);
        ...
        for(int index=0;index<Params.SERVICE_REQUEST_COUNT;index++) {
            ...
            try {
                ...
                httpTransport.call(ServicesConstants.WS_ACTION+ServicesConstants.WS_METHOD_ALL_SELLING_STORES, envelope);
                break;
            } catch (Exception e) {
                ErrorMessageInfo bean = new ErrorMessageInfo();
    ...
    private Object parseSOAPResponse(SoapObject response){
        String status = response.getPrimitivePropertySafelyAsString("SelectCardSellingStores");
