...
/**
 * Created by WIN on 4/23/2016.
 */
public class TotalCashPointsAsync extends AbstractAsyncTask<Object, Void, Object> {
    ...
    public TotalCashPointsAsync(IServiceListener listener, ArrayList<ServiceRequest> list, int processType){
    ...
    private Object callService() {
        ...
        for(int index=0;index<Params.SERVICE_REQUEST_COUNT;index++) {
            try {
                ...
                httpTransport.call(ServicesConstants.WS_ACTION+ServicesConstants.WS_METHOD_GET_TOTAL_POINTS, envelope);
                break;
            }
            catch (Exception e) {
                ErrorMessageInfo bean = new ErrorMessageInfo();
    ...
    private Object parseSOAPResponse(SoapObject response){
        String status = response.getPrimitivePropertySafelyAsString("GetTotalPointsResult");
        ErrorMessageInfo bean = new ErrorMessageInfo();
    ...
    public SoapObject createRequest() {
        SoapObject request = new SoapObject(ServicesConstants.WS_NAME_SPACE, ServicesConstants.WS_METHOD_GET_TOTAL_POINTS);
