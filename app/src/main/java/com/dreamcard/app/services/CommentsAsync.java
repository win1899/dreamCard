 ...
 */
public class CommentsAsync extends AbstractAsyncTask<Object, Void, Object> {
    ...
    public CommentsAsync(IServiceListener listener,ArrayList<ServiceRequest> list,int processType,int type){
    ...
    private Object callService(){
        ...
        String method=ServicesConstants.WS_METHOD_OFFER_COMMENTS;
        if(this.type==Params.TYPE_BUSINESS)
            method=ServicesConstants.WS_METHOD_BUSINESS_COMMENTS;
                ...
                if(envelope.getResponse() instanceof SoapPrimitive){
                    ...
                    try {
                        ...
                        for (int i=0; i < jArray.length(); i++){
                            ...
                            if(oneObject.getString("Comment")!=null
                                    && !oneObject.getString("Comment").equalsIgnoreCase("")
                                    && oneObject.getString("Comment").length()>0) {
                                Comments bean = new Comments();
                                bean.setId(oneObject.getString("Id"));
                                bean.setComment(oneObject.getString("Comment"));
                                bean.setConsumer(oneObject.getString("consumerName"));
                                list.add(bean);
                            }
                        }
                        return list;
