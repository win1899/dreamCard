...
public class ConsumerDiscountAsyncTask extends AsyncTask<Object, Void, Object> {
    ...
    public ConsumerDiscountAsyncTask(IServiceListener listener,ArrayList<ServiceRequest> list,int processType){
    ...
    private Object callService(){
        ...
        String method= ServicesConstants.WS_METHOD_CONSUMER_DISCOUNT;
                ...
                if(envelope.getResponse() instanceof SoapPrimitive){
                    ...
                    try {
                        ...
                        for (int i=0; i < jArray.length(); i++){
                            ...
                            String offerUsage=oneObject.getString("OfferUsage");
                            if(offerUsage!=null
                                    && !offerUsage.equalsIgnoreCase("null")) {
                            ...
                            String offer=oneObject.getString("Offer");
                            if(offer!=null
                                    && !offer.equalsIgnoreCase("null")) {
                                JSONObject offerObject = oneObject.getJSONObject("Offer");


                                bean.setId(offerObject.getString("Id"));
                                bean.setType(offerObject.getString("Type"));
                                bean.setBusinessId(offerObject.getString("BusinessId"));
                                bean.setDescription(offerObject.getString("Description"));
                                bean.setSaleOldPrice(offerObject.getString("SaleOldPrice"));
                                bean.setStatus(offerObject.getString("Status"));
                                bean.setTitle(offerObject.getString("Title"));
                                bean.setValidationPeriod(offerObject.getString("ValidationPeriod"));
                                bean.setValidFrom(offerObject.getString("ValidFrom"));
                                bean.setCurrency(offerObject.getString("Currency"));
                                bean.setOfferMainPhoto(offerObject.getString("MainPicture"));
                                bean.setNumOfLikes(offerObject.getString("LikesCount"));
                                String pictures=offerObject.getString("Pictures");
                                if(pictures!=null && !pictures.equalsIgnoreCase("null") && pictures.length()>0){
                                    bean.setPicturesList(pictures.split(";"));
                                }

                                if(offerObject.getString("SaleNewPrice")==null
                                        || offerObject.getString("SaleNewPrice").equalsIgnoreCase("null"))
                                    bean.setSaleNewPrice("");
                                else
                                    bean.setSaleNewPrice(offerObject.getString("SaleNewPrice"));
                                if(offerObject.getString("DiscountPercentage")==null
                                        || offerObject.getString("DiscountPercentage").equalsIgnoreCase("null"))
                                    bean.setDiscount("");
                                else
                                    bean.setDiscount(offerObject.getString("DiscountPercentage")+"%");

                                if(offerObject.getString("Rating")==null
                                        || offerObject.getString("Rating").equalsIgnoreCase("null")) {
                                    bean.setOfferRating(0);
                                }else {
                                    bean.setOfferRating(offerObject.getInt("Rating"));
                                }

                                if(offerObject.getString("RatingCount")==null
                                        || offerObject.getString("RatingCount").equalsIgnoreCase("null")) {
                                    bean.setRatingCount(0);
                                }else {
                                    bean.setRatingCount(offerObject.getInt("RatingCount"));
                                }
