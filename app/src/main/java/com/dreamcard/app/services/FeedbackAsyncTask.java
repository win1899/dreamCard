 ...
 */
public class FeedbackAsyncTask extends AbstractAsyncTask<Object, Void, Object> {
    ...
    public FeedbackAsyncTask(IServiceListener listener,ArrayList<ServiceRequest> list,int processType){
    ...
    private Object callService(){
        ...
        for(int index=0;index<Params.SERVICE_REQUEST_COUNT;index++) {
            try {
                ...
                httpTransport.call(ServicesConstants.WS_ACTION+ServicesConstants.WS_METHOD_FEEDBACK, envelope);
                break;
            } catch (Exception e) {
                ErrorMessageInfo bean = new ErrorMessageInfo();
...
//        }
    ...
    private Object parseSOAPResponse(Vector list) {
        if(list.size()>0){
            ...
            String status = returnedValueNode.getPrimitivePropertySafelyAsString("SubmitFeedbackResult");
    ...
    public SoapObject createRequest() {
        SoapObject request = new SoapObject(ServicesConstants.WS_NAME_SPACE, ServicesConstants.WS_METHOD_FEEDBACK);
